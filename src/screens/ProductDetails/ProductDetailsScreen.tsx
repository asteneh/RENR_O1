...
export default function ProductDetailsScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.ScrollView
        contentContainerStyle={{ paddingTop: HEADER_HEIGHT - 30, paddingBottom: 100 }} // Added paddingBottom for button
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.headerInfo}>
            ...
            <Text style={styles.price}>
              ETB {(product.currentPrice && typeof product.currentPrice === 'number') ? product.currentPrice.toLocaleString() : 'N/A'}
            </Text>

            <View style={styles.badgeRow}>
              <View style={[styles.badge, { backgroundColor: THEME_COLOR }]}>
                <Text style={styles.badgeText}>{product.postType?.name || 'For Sale'}</Text>
              </View>
              <View style={styles.locationRow}>
                <Ionicons name="location-outline" size={16} color="#666" />
                <Text style={styles.locationText}>{product.location?.descripton || 'Addis Ababa'}</Text>
              </View>
            </View>
          </View>
            ...
            <>
              ...
              <View style={styles.specsContainer}>
                {product.attributes.slice(0, 4).map((attr, index) => (
                  <SpecItem key={index} icon="information-outline" label={attr.name} value={attr.value} />
                ))}
              </View>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  specsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 20 }, // Changed to wrap for dynamic specs
  specBox: {
    width: (width - 60) / 4, // Calculate width dynamically
    backgroundColor: '#FAFAFA',
    padding: 10, borderRadius: 12,
    alignItems: 'center', borderWidth: 1, borderColor: '#EEE'
  },
  specValue: { fontSize: 12, fontWeight: 'bold', marginTop: 5, color: '#333' },
  ...
  galleryImg: { width: 80, height: 60, borderRadius: 8, marginRight: 10, backgroundColor: '#eee' }, // Smaller thumbnails
